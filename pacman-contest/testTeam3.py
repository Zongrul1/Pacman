...
class MCTCaptureAgent(CaptureAgent):
    ...
    def bfs(self, gameState, goal):
        ...
        while not openList.isEmpty():
            ...
            print
            "state: " + str(curPos) + " goal: " + str(goal)
            if curPos == goal:
                print
                "arrive goal: " + str(path.pop())
...
class OffensiveMCTAgent(MCTCaptureAgent):
    ...
    def chooseAction(self, gameState):
        ...
        if self.survivalMode is False:
            ...
            print
            'eval time for agent %d: %.4f' % (self.index, time.time() - start)
    ...
    def survivalCheck(self, gameState):
        ...
        if self.survivalMode and self.survivalPoint == (0, 0):
            ...
            print
            "set the survivalPoint: ", self.survivalPoint
    ...
    def goBackHome(self, gameState):
        ...
        bestAction = None
        enemies = [gameState.getAgentState(i) for i in self.getOpponents(gameState)]
        ghostPos = [a.getPosition() for a in enemies if
                    not a.isPacman and a.getPosition() != None and a.scaredTimer < 5]
        ...
        for action in actions:
            ...
            if dist == 0:
                print
                "reset survivalPoint !", self.survivalPoint
                self.survivalPoint = (0, 0)
                break
    ...
    def getWeights(self, gameState, action):
        ...
        # print "-----Normal Mode-----"
        return {'arrivedHome': 100, 'successorScore': 150, 'distanceToFood': -5, 'reverse': -3, 'distanceToEntry': -10,
                'dead': -999, 'deadEnd': -200, 'eatenCap': 200}
...
class DefensiveMCTAgent(MCTCaptureAgent):
    ...
    def registerInitialState(self, gameState):
        ...
        if self.red:
            for central in range(int((gameState.data.layout.width - 2) / 2), 0, -1):
                if not gameState.hasWall(central, height):
            ...
            for central in range(int((gameState.data.layout.width - 2) / 2 + 1), gameState.data.layout.width, 1):
                if not gameState.hasWall(central, height):
                    self.detectDest = [(central, height)]
                    i = i + 1
                    if i == 2:
...
class Node:
    ...
    def printNode(self):
        ...
        self.gameState
        print
        "reward: ", self.reward, " cnt: ", self.cnt
